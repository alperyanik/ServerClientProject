...
{
    /// <summary>
    /// Manuel AES-128 Implementasyonu - Eğitim Amaçlı (Server - Decrypt)
    /// Inverse S-Box, InvShiftRows, InvMixColumns ve 10 round içerir.
    /// </summary>
    public static class ManualAESCipher
    {
        ...
        private static void InvMixColumns(byte[] state)
        {
            for (int c = 0; c < 4; c++)
            {
                ...
                state[i] = (byte)(GFMul(0x0e, s0) ^ GFMul(0x0b, s1) ^ GFMul(0x0d, s2) ^ GFMul(0x09, s3));
                state[i + 1] = (byte)(GFMul(0x09, s0) ^ GFMul(0x0e, s1) ^ GFMul(0x0b, s2) ^ GFMul(0x0d, s3));
                state[i + 2] = (byte)(GFMul(0x0d, s0) ^ GFMul(0x09, s1) ^ GFMul(0x0e, s2) ^ GFMul(0x0b, s3));
                state[i + 3] = (byte)(GFMul(0x0b, s0) ^ GFMul(0x0d, s1) ^ GFMul(0x09, s2) ^ GFMul(0x0e, s3));
            }
        }
        ...
        public static string Decrypt(string cipherText, byte[] key)
        {
            ...
            {
                return "[Manuel AES Çözme Hatası]";
            }
        }
